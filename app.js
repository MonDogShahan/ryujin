...
const App = () => {
    ...
    // 確保資料庫載入
    useEffect(() => {
        if (window.AC_DATABASE && window.AC_DATABASE.length > 0) {
            setDbLoaded(true);
        } else {
            // 如果還沒載入，每 100ms 檢查一次
    ...
    // 白屏防護：資料庫未載入時顯示 Loading
    if (!dbLoaded) {
        return <div className="min-h-screen bg-industrial-950 flex flex-col justify-center items-center text-yellow-500 font-bold"><div className="animate-spin mb-4 text-4xl">⚙</div><div>系統初始化中...</div></div>;
    }
                    ...
                    <div className="animate-fade-in h-full flex flex-col">
                        <div className="bg-industrial-800 p-4 rounded-xl border border-industrial-700 shadow-inner mb-4 space-y-4 shrink-0">
                            ...
                            <div className="relative pt-2" ref={searchContainerRef}>
                                ...
                                <input type="text" className="w-full pl-9 pr-8 py-3 bg-industrial-900 border border-industrial-700 rounded-lg text-sm text-white focus:border-industrial-accent transition-all" placeholder="輸入 28, NJP..." value={searchState.keyword} onChange={e => setSearchState(p => ({...p, keyword: e.target.value}))} onFocus={() => setShowHistory(true)} onKeyDown={e => e.key === 'Enter' && executeSearch()} />
