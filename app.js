...
const App = () => {
    ...
    // ... (availableSeries, getFilteredResults, executeSearch 保持不變，直接引用上一版邏輯) ...
    const availableSeries = useMemo(() => { if (searchState.brand === '不拘') return ['不拘']; return ['不拘', ...new Set(AC_DATABASE.filter(i => i.brandCN === searchState.brand).map(i => i.series))]; }, [searchState.brand]);
    const getFilteredResults = (kw) => { return AC_DATABASE.filter(i => { if (searchState.brand !== '不拘' && i.brandCN !== searchState.brand) return false; if (searchState.series !== '不拘' && i.series !== searchState.series) return false; if (searchState.func !== '不拘' && i.func !== searchState.func) return false; if (searchState.type !== '不拘' && i.type !== searchState.type) return false; if (kw) { const kL = kw.toLowerCase(); const s = JSON.stringify(i).toLowerCase(); if (!isNaN(parseFloat(kw)) && isFinite(kw)) { const n = parseFloat(kw); const cap = Math.abs(parseFloat(i.maxKw)*10 - n) < 0.5 || Math.abs(parseFloat(i.maxKw) - n) < 0.1; const mod = i.modelIdu.toLowerCase().includes(kL) || i.modelOdu.toLowerCase().includes(kL); return cap || mod; } return s.includes(kL); } return true; }); };
    const executeSearch = () => { const f = getFilteredResults(searchState.keyword); const g = []; f.forEach(i => { const k = `${i.brandCN}-${i.series}-${i.maxKw}`; let gp = g.find(x => `${x.brandCN}-${x.series}-${x.maxKw}` === k); if (!gp) { gp = { ...i, variants: [] }; g.push(gp); } gp.variants.push(i); }); setSearchState(p => { const h = p.keyword && !p.history.includes(p.keyword) ? [p.keyword, ...p.history].slice(0, 5) : p.history; localStorage.setItem('searchHistory', JSON.stringify(h)); return { ...p, results: g.sort((a,b) => a.maxKw - b.maxKw), history: h }; }); setShowHistory(false); };

    // ★ 關鍵修正 1: 搜尋建議邏輯優化 (只抓取型號IDU匹配的字串，不亂跳)
    const suggestions = useMemo(() => {
        if (!searchState.keyword) return searchState.history;
        const inputLower = searchState.keyword.toLowerCase();
        
        // 只篩選 "型號 (modelIdu)" 中包含輸入關鍵字的項目
        const matches = AC_DATABASE
            .filter(i => i.modelIdu.toLowerCase().includes(inputLower))
            .map(i => i.modelIdu)
            .slice(0, 5); // 最多顯示5筆
    ...
    // 移除 click outside 關閉功能，改為點選建議後自動關閉
                ...
                {activeTab === 'search' && (
                    <div className="animate-fade-in h-full flex flex-col">
                        <div className="bg-industrial-800 p-4 rounded-xl border border-industrial-700 shadow-inner mb-4 space-y-4 shrink-0">
                            ...
                            {/* ★ 關鍵修正 2: 版面調整 - 建議選單改為「Block (推擠)」而非「Absolute (覆蓋)」 */}
                            <div className="relative pt-2">
                                ...
                                <input 
                                    ...
                                    placeholder="輸入型號 (如 28, NJP)..." 
                                    value={searchState.keyword} 
                                    onChange={e => { setSearchState(p => ({...p, keyword: e.target.value})); setShowHistory(true); }}
                            ...
                            {/* 建議選單移到這裡，直接渲染在文件流中，會自動把下方的按鈕推下去，不會擋住 */}
                            {showHistory && suggestions.length > 0 && (
                                <div className="bg-industrial-900 border border-industrial-700 rounded-lg overflow-hidden animate-slide-up mb-2">
                                    <div className="max-h-40 overflow-y-auto">
                                        {suggestions.map((h, i) => (
                                            <button key={i} onClick={() => { setSearchState(p => ({...p, keyword: h})); executeSearch(); setShowHistory(false); }} className="w-full text-left px-4 py-3 text-sm text-gray-300 hover:bg-industrial-800 border-b border-gray-800 last:border-0 flex items-center justify-between group">
                                                <span>{h}</span>
                                                <Icon name="chevron" className="w-3 h-3 text-gray-600 group-hover:text-white" />
                                            </button>
                                        ))}
                                    </div>
                                    <div className="bg-industrial-950 p-2 text-right"><button onClick={clearHistory} className="text-xs text-red-400 hover:text-red-300 flex items-center justify-end gap-1 w-full"><Icon name="trash" className="w-3 h-3"/> 清除紀錄</button></div>
                                </div>
                            )}
