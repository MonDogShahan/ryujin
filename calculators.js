...
const generateProfessionalPDF = (title, items, summary) => {
    ...
    <head>
        <title>${title}</title>
        <style>
            body { font-family: "Microsoft JhengHei", "Heiti TC", sans-serif; padding: 40px; color: #333; }
            ...
            .advice-text { white-space: pre-wrap; font-size: 11px; line-height: 1.5; margin-top: 4px; }
            .math-detail { font-family: Consolas, monospace; font-size: 10px; color: #64748b; }
                ...
                {rooms.map((room) => (
                    <div key={room.id} className="bg-industrial-800 p-4 rounded-xl border border-industrial-700 shadow-sm relative">
                        ...
                        <div className="flex flex-wrap gap-2 mb-2">
                            {Object.entries({ westSun: '西曬', allDaySun: '全日曬', topFloor: '頂樓', highCeiling: '挑高', ironSheet: '鐵皮' }).map(([k, l]) => (
...
const DuctedCalculator = ({ plans, setPlans, db }) => {
    ...
    const calculatePlan = (id) => {
        setPlans(plans.map(p => {
            ...
            // 計算細節 (只顯示整數)
            const details = [];
            details.push(`8"管 (${area8}cm²): 可接 ${Math.floor(flangeArea/area8)} 孔`);
            ...
            // ★ 核心邏輯修正：判斷是否需要分歧
            if (outlets >= 2) {
                // 如果法蘭小於「2孔8吋 (628cm²)」 -> 代表法蘭太小，塞不下兩個8吋
                if (flangeArea < 600) { // 稍微給點容許值 (628 * 0.95)
                    statusColor = "text-yellow-400";
                    advice = `⚠ 法蘭面積 (${flangeArea}cm²) 不足直接開 ${outlets} 孔 8"！\n\n✅ 建議配置 (三通分流)：\n1. 集風箱只開 1 孔主管 (建議 10" 或 12")。\n2. 延伸後接三通 (Y管) 分出 ${outlets} 孔 8"。\n(此做法可利用主管容積穩壓，避免法蘭擁擠)`;
                } else {
                    // 法蘭夠大 (>= 600)，可以直接開孔
                ...
                <tbody>
                    ${plans.map(p => {
                        ...
                        return `<tr>
                            ...
                            <td>${res && !res.error ? `<div class="math-detail">${res.details.join('<br/>')}</div><div class="advice-text" style="color:${res.statusColor === 'text-red-400' ? 'red' : '#059669'}">${res.advice}</div>` : '<span style="color:gray">未試算</span>'}</td>
                        </tr>`;
                    }).join('')}
                </tbody>
            </table>`;
    ...
    return (
        <div className="animate-fade-in pb-10 space-y-4">
            ...
            {showReset && <div className="bg-red-900/30 border border-red-500/50 p-3 rounded-lg text-center animate-zoom-in"><p className="text-xs text-red-200 mb-2">確定重置？</p><div className="flex gap-2 justify-center"><button onClick={resetAll} className="px-3 py-1 bg-red-600 text-white rounded text-xs">確定</button><button onClick={() => setShowReset(false)} className="px-3 py-1 bg-gray-600 text-white rounded text-xs">取消</button></div></div>}
            ...
            {plans.map((plan, idx) => (
                <div key={plan.id} className="bg-industrial-800 p-4 rounded-xl border border-industrial-700 shadow-xl relative animate-slide-up">
                    ...
                    <div className="grid grid-cols-2 gap-3 mt-4 mb-3">
                        <div className="relative"><span className="absolute -top-2 left-2 bg-industrial-800 px-1 text-[8px] text-gray-400">品牌</span><select value={plan.brand} onChange={e => updatePlan(plan.id, 'brand', e.target.value)} className="w-full bg-industrial-900 border border-industrial-600 rounded p-2 text-xs text-white outline-none"><option value="">請選擇</option>{brands.map(b => <option key={b} value={b}>{b}</option>)}</select></div>
                        <div className="relative"><span className="absolute -top-2 left-2 bg-industrial-800 px-1 text-[8px] text-gray-400">型號</span><select value={plan.model} onChange={e => updatePlan(plan.id, 'model', e.target.value)} className="w-full bg-industrial-900 border border-industrial-600 rounded p-2 text-xs text-white outline-none" disabled={!plan.brand}><option value="">請選擇</option>{db.filter(m => m.brandCN === plan.brand && m.type === '吊隱式').sort((a,b)=>a.maxKw-b.maxKw).map(m => (<option key={m.id} value={m.id}>{m.modelIdu} ({m.maxKw}kW)</option>))}</select></div>
                    </div>
                    <div className="grid grid-cols-3 gap-2 mb-3">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => calculatePlan(plan.id)} className="flex-1 bg-blue-600 hover:bg-blue-500 text-white rounded p-2 text-xs font-bold">開始試算</button>
                    </div>
            ...
            <button onClick={addPlan} className="w-full py-3 border border-dashed border-gray-600 text-gray-400 rounded-xl text-sm hover:text-white">+ 新增一台主機</button>
        </div>
    );
};
