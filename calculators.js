                ...
                <div key={p.id} className="bg-industrial-800 p-5 rounded-xl border border-industrial-700 relative">
                    ...
                    <div className="mb-4 relative">
                         ...
                         <div className="flex gap-2">
                             <input type="text" placeholder={p.model || "例如: FBA50, 大金, 63..."} className="w-full bg-industrial-900 border border-gray-600 rounded-lg px-3 py-2 text-sm text-white focus:border-industrial-accent" onChange={(e) => handleSearch(e.target.value, p.id)} value={activePlanId === p.id ? searchKw : p.model} onFocus={() => { setActivePlanId(p.id); setSearchKw(p.model || ''); }} />
                         </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-4">
                        <div><label className="text-[10px] text-gray-500">法蘭寬 (mm)</label><input type="number" value={p.flangeW} onChange={e => updatePlan(p.id, 'flangeW', e.target.value)} className="w-full bg-industrial-900 border border-gray-700 rounded p-2 text-white" placeholder="自帶數據" /></div>
                        <div><label className="text-[10px] text-gray-500">法蘭高 (mm)</label><input type="number" value={p.flangeH} onChange={e => updatePlan(p.id, 'flangeH', e.target.value)} className="w-full bg-industrial-900 border border-gray-700 rounded p-2 text-white" placeholder="自帶數據" /></div>
                    ...
                    {p.result && (
                        <div className="bg-black/30 rounded p-3 text-xs space-y-1 border border-industrial-600/30">
                            ...
                            <div className="mt-2 pt-2 border-t border-gray-700"><span className="text-gray-400 block mb-1">建議出風口總面積 (單口)</span><span className="text-industrial-accent font-mono text-sm block text-center">{Math.round(p.result.suggestW / p.outlets)} x {p.result.suggestH} mm</span></div>
                        </div>
                    )}
                </div>
            ))}
