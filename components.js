// ================= components.js (V13.50 經典彈窗+新規格支援版) =================
// 包含：Icon圖示, 下拉選單, 結果卡片(點擊彈窗), 詳細規格視窗
...
// 3. 結果卡片 (ResultCard) - 點擊觸發 onClick (開啟 Modal)
    ...
    return (
        ...
        >
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-xs border-t border-industrial-700 pt-3">
                ...
                <div className="flex justify-between"><span className="text-gray-500">管徑</span><span className="text-gray-200 font-mono">{main.pipes}</span></div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fade-in">
            ...
            <div className="bg-industrial-900 w-full max-w-md rounded-2xl shadow-2xl border border-industrial-700 flex flex-col max-h-[90vh] relative z-10 animate-zoom-in">
                ...
                <div className="flex-1 overflow-y-auto p-5 custom-scroll">
                    {/* 功能切換 (若該型號同時有冷暖/冷專才顯示，目前資料結構多為分開，此處保留邏輯) */}
                    {group.variants.length > 1 && (
                        <div className="flex bg-industrial-950 p-1 rounded-lg mb-6 border border-industrial-800">
                            {group.variants.map((v, i) => (
                                <button key={i} onClick={() => setCurrentFunc(v.func)} className={`flex-1 py-2 text-xs font-bold rounded-md transition-all ${currentFunc === v.func ? (v.func === '冷暖' ? 'bg-industrial-700 text-white shadow' : 'bg-cool-900 text-cool-100 shadow') : 'text-gray-500 hover:text-gray-300'}`}>
                                    {v.func} ({v.maxKw}kW)
                                </button>
                            ))}
                        </div>
                    ...
                    <div className="space-y-0.5">
                        ...
                        <SpecRow label="電源配置" value={val(activeData.powerConn)} highlight />
                        <SpecRow label="運轉電壓" value={val(activeData.volts)} highlight />
                        <SpecRow label="運轉電流" value={`${val(activeData.current)} A`} />
                        ...
                        <div className="h-2"></div>
                        <div className="text-[10px] text-gray-500 font-bold mb-1 mt-2 pl-1">尺寸規格 (寬x高x深 mm)</div>
                        
                        {!isMulti && (
                            <div className="bg-industrial-800/50 rounded-lg p-3 mb-2 border border-industrial-700/50">
